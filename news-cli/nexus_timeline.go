...
func RenderNexusTimeline(entityName string, articles []Article) string {
	if len(articles) == 0 {
	...
	doc.WriteString(titleStyle.Render("EVOLUTION OF: "+strings.ToUpper(entityName)) + "\n\n")
...
// GenerateNexusSummary provides a high-level summary of the entity's history.
func GenerateNexusSummary(entityName string, articles []Article) string {
	if len(articles) < 2 {

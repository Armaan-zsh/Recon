...
```bash
go install github.com/spacecadet/recon@latest
```

### Option 2: One-liner (download pre-built binary)

```bash
curl -sL https://github.com/spacecadet/recon/releases/latest/download/recon-linux-amd64 -o ~/.local/bin/recon && chmod +x ~/.local/bin/recon
```
